...
//  QueueApp_1.0
//
//  Created by Patrick Cockrill on 6/18/19.
...
//        let isFormValid = emailTextField.text?.isEmpty == false && passwordTextField.text?.isEmpty == false
        let isPostValid = itemTitle.text?.isEmpty == false && priceTitle.text?.isEmpty == false && categoryTitle.text?.isEmpty == false && descriptionText.text.isEmpty == false
        if isPostValid {
            navigationItem.rightBarButtonItem?.isEnabled = true
            shareButton.isEnabled = true
            shareButton.textColor = .white
            
            saveImageDetailsToStorage()
            //        saveItemDetailsToDatabaseWithUrl(images: imageUrlArray)
            ...
            shareButton.textColor = UIColor(white: 1, alpha: 0.5)
        }
    }
    ...
    fileprivate func saveImageDetailsToStorage() {
        ...
        guard let description = descriptionText.text, !description.isEmpty else { return }
        ...
        for image in images {
            ...
            storageRef.putData(uploadData, metadata: nil) { (metadata, error) in
                ...
                storageRef.downloadURL(completion: { (downloadURL, error) in
                    ...
                    // Call helper function of saving to databse (title, price, description, etc..)
    ...
    fileprivate func saveItemDetailsToDatabaseWithUrl(images: [String]) {
        ...
        guard let category = categoryTitle.text else { return }
        guard let description = descriptionText.text else { return }
        
        
        guard let uid = Auth.auth().currentUser?.uid else { return }
        
        // userPostRef gets the user's uid
        let userPostRef = Database.database().reference().child("posts").child(uid)
        // This generates a random post id to save in the database
        let ref = userPostRef.childByAutoId()
        
        let values = ["creationDate": Date().timeIntervalSince1970, "postImageUrl": images, "postTitle": title, "price": price, "category": category, "description": description] as [String: Any]
