...
//  QueueBeta
//
//  Created by Patrick Cockrill on 9/12/19.
...
class ItemOverviewCell: UICollectionViewCell {
    ...
    override init(frame: CGRect) {
        ...
        circleView.backgroundColor = .rgb(red: 236, green: 236, blue: 239)
        circleView.addSubview(sendMessageButton)
        circleView.clipsToBounds = true
        circleView.layer.cornerRadius = 25
        sendMessageButton.fillSuperview(padding: .init(top: 8, left: 12.5, bottom: 8, right: 12.5))
        
        let namePriceStack = VerticalStackView(arrangedSubviews: [nameLabel, priceLabel, listedCity], spacing: 8)
        let buttonOptionStack = UIStackView(arrangedSubviews: [circleView, shareItemButton], customSpacing: 0)
        buttonOptionStack.distribution = .equalCentering
        buttonOptionStack.axis = .horizontal
        
        addSubview(namePriceStack)
        addSubview(buttonOptionStack)
        
        namePriceStack.anchor(top: topAnchor, left: leftAnchor, bottom: nil, right: rightAnchor, paddingTop: 16, paddingLeft: 20, paddingBottom: 8, paddingRight: 20, width: 0, height: 0)
        buttonOptionStack.anchor(top: namePriceStack.bottomAnchor, left: nil, bottom: bottomAnchor, right: nil, paddingTop: 10, paddingLeft: 0, paddingBottom: 8, paddingRight: 0, width: frame.width / 3, height: 50)
        buttonOptionStack.centerXAnchor.constraint(equalTo: centerXAnchor).isActive = true
